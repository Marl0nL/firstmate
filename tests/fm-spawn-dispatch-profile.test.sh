...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  # This case reuses the one slot across two sequential spawns purely to compare
  # path spellings; the first task is done being measured, so release its lease
  # (as teardown would) before the second spawn, or fm-spawn's worktree-lease
  # guard correctly refuses a second live task into a slot the first still owns.
