...
# The stand-in launch command must outlive the whole suite: the treehouse pool
# judges a slot free by whether a live process sits inside it, and a slot whose
# process died while its task's meta persists would be re-leased to a later
# spawn - which fm-spawn.sh's worktree-lease guard then refuses, naming the
# still-recorded owner. A short sleep made that a timing lottery.
spawn_task() {  # <id> <home> <project>
  local id=$1 home=$2 project=$3
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$project" "sh -c 'sleep 7200'" --mode no-mistakes --yolo off --backend herdr
}
...
spawn_secondmate_task() {
  ...
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$HOME_DIR" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$home" "sh -c 'sleep 7200'" --secondmate --backend herdr
}
...
# The deliberate stop+provision restart scenarios below kill every leased
# slot's process, so a spawn after a restart would be handed the lowest
# freed slot of a shared pool - one an unrelated same-home task's meta still
# records, which fm-spawn.sh's worktree-lease guard loudly refuses. Give the
# restart scenarios their own project (their own treehouse pool) so the only
# recorded lease a post-restart acquisition can collide with is the reclaimed
# task's own, which the guard's same-identity exemption permits.
...
# The restart-scenario pool again: every shared-pool slot's process died at the
# last stop+provision while p1/p2/pcw/anchor still record those slots, so a
# shared-pool acquisition here would be refused by the worktree-lease guard.
