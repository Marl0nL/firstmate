...
# (N=2), or "always unverifiable" (pre==alive==node). The window name is echoed
# back for list-windows and #{pane_tty} is empty, so the trunk's multi-source
# tmux liveness classifier resolves the verdict from the foreground command.
set -u
...
TMP_ROOT=$(fm_test_tmproot fm-spawn-agent-confirm)

# A fake tmux driven by FM_FAKE_STATE/literal-count (incremented per launch send)
# and env knobs FM_FAKE_ALIVE_AFTER (N), FM_FAKE_PRE_CMD, FM_FAKE_ALIVE_CMD, and
# FM_FAKE_WINDOW (the pane's window name, so the classifier's inventory check
# finds it).
...
case "$*" in
  *"#{pane_current_command}"*)
    ...
    exit 0
    ;;
  *"#{pane_tty}"*) exit 0 ;;                                   # empty tty -> no foreground probe
  *"#{pane_current_path}"*) printf '%s\n' "${FM_FAKE_PANE_PATH:-}"; exit 0 ;;
  *"#{pane_id}"*) printf '%%0\n'; exit 0 ;;
  # Both the pre-creation duplicate check and the post-launch liveness classifier
  # call `list-windows -t <session> -F '#{window_name}'`. Report the window only
  # AFTER new-window has "created" it (a marker), so the pre-creation check sees
  # no duplicate yet the liveness inventory check finds it and the verdict comes
  # from the foreground command above.
  *"#{window_name}"*)
    [ -e "$state/window-created" ] && printf '%s\n' "${FM_FAKE_WINDOW:-}"
    exit 0
    ;;
esac
case "${1:-}" in
  ...
  new-window) : > "$state/window-created"; printf '@1\n'; exit 0 ;;
  send-keys)
    prev=
    for a in "$@"; do
      if [ "$prev" = "-l" ]; then
...
SH
  ...
  # A pi probe so the unverifiable-harness case can spawn pi at all; pi's
  # spawn preflight checks its --help version.
  cat > "$fakebin/pi" <<'SH'
#!/usr/bin/env bash
set -u
if [ "${1:-}" = --help ]; then
