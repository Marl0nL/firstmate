...
# spawn any worktree it likes, and this home's own metas supply the competitors.
set -u
...
# A fake tmux that reports FM_FAKE_PANE_PATH as the pane cwd (so the spawn-time
# path poll resolves immediately to that worktree) and logs every literal
# `send-keys -l` launch line, so a test can prove an aborted spawn launched
# nothing.
make_lease_fakebin() {
  local dir=$1 fakebin
...
# make_case <name> -> record "case_dir|home|proj|wt_a|wt_b|fakebin|launchlog"
# Two real worktrees of one project: wt_a is the contested slot, wt_b a free one.
make_case() {
  local name=$1 case_dir home proj wt_a wt_b fakebin launchlog
  ...
  : > "$launchlog"
  printf '%s\n' "$case_dir|$home|$proj|$wt_a|$wt_b|$fakebin|$launchlog"
}

read_case() {
  IFS='|' read -r CASE_DIR HOME_DIR PROJ_DIR WT_A WT_B FAKEBIN_DIR LAUNCH_LOG <<EOF
$1
EOF
}
...
run_spawn() {
  ...
  run_spawn_raw "$rec" "$pane_path" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off
}
...
# A non-empty launch log means a launch command was actually sent.
...
test_refuses_worktree_owned_by_another_task() {
  ...
  ! launched || fail "an aborted spawn must not send a launch"
...
test_refuses_secondmate_home_owned_by_another_task() {
  ...
  fm_write_meta "$HOME_DIR/state/lease-sm-owner-f6.meta" \
    ...
    "kind=secondmate" \
    "home=$sm"
  out=$(run_spawn_raw "$rec" "$sm" lease-sm-intruder-g7 "$sm" --secondmate); status=$?
  ...
  ! launched || fail "an aborted secondmate spawn must not send a launch"
