...
#       including a bundled short-option cluster that carries -R, and --hostname
#       fails fast too because the host comes only from the URL (github.com);
#       the github merge call is pinned to GH_HOST=github.com
...
#!/usr/bin/env bash
# Record the child GH_HOST on its own line so a test can prove fm-pr-merge pins
# it to github.com; the argv line below stays exact for the other assertions.
printf 'GH_HOST=%s\n' "${GH_HOST:-<unset>}" >> "$FM_TEST_GH_AXI_LOG"
printf '%s\n' "$*" >> "$FM_TEST_GH_AXI_LOG"
exit 0
SH
...
test_records_pr_and_head_before_merging() {
  ...
  # The github merge call pins GH_HOST=github.com so an ambient or inherited host
  # can never redirect the merge off the host the PR URL named.
  grep -qxF 'GH_HOST=github.com' "$case_dir/gh-axi.log" \
    || fail "records-before-merge: gh-axi pr merge was not pinned to GH_HOST=github.com"
  pass "fm-pr-merge records pr= and pr_head= before invoking gh-axi pr merge, pinned to github.com"
}
...
# --hostname is a gh-axi/gh global flag that sets GH_HOST for the child gh, so an
# extra --hostname arg would merge the recorded PR against a DIFFERENT GitHub host
# while state still records the github.com URL. Both the repo and the host come
# only from the PR URL, which this script accepts only on github.com, so the guard
# must refuse both spellings before anything is recorded or read.
assert_hostname_override_refused() {  # <case-name> <extra merge args...>
  local case_name=$1 case_dir rc
  shift
  case_dir=$(make_case "hostname-override-$case_name")
  mkdir -p "$case_dir/wt"
  add_gh_mocks "$case_dir" cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd
  : > "$case_dir/gh-axi.log"

  set +e
  run_pr_merge "$case_dir" task-x1 https://github.com/right/repo/pull/7 -- "$@" \
    > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e

  expect_code 1 "$rc" "hostname-override ($case_name): fm-pr-merge should refuse a host override"
  assert_grep 'must not override the repository or host parsed from the PR URL' "$case_dir/stderr" \
    "hostname-override ($case_name): refusal did not explain the host override"
  assert_no_grep 'pr=https://github.com/right/repo/pull/7' "$case_dir/state/task-x1.meta" \
    "hostname-override ($case_name): PR URL was recorded before rejecting the host override"
  assert_absent "$case_dir/state/task-x1.check.sh" \
    "hostname-override ($case_name): a host override armed a merge poll"
  assert_no_grep 'pr merge' "$case_dir/gh-axi.log" \
    "hostname-override ($case_name): gh-axi pr merge was invoked despite the host override"
}

test_hostname_override_args_refuse_before_recording() {
  assert_hostname_override_refused space-form --hostname other.example
  assert_hostname_override_refused equals-form --hostname=other.example
  pass "fm-pr-merge refuses --hostname host overrides before recording state"
}
