...
test_wire_unwire_shim() {
  ...
  # shellcheck disable=SC2016  # $FM_ROOT/$FM_HOME are expanded by the inner bash, not here.
  env FM_HOME="$home" FM_ROOT="$ROOT" bash -c \
    '. "$FM_ROOT/bin/fm-crowsnest-lib.sh"; fm_custom_check_registered "$FM_HOME/state" chat-watch' >/dev/null \
