...
- **Optional Crowsnest** - opt in with one local `config/crowsnest.env` so you can reach the single live firstmate session from a Google Chat thread and it can post back, all without spawning a second fleet-aware agent; a chat message becomes a wake the one live session answers on its own turn, and non-Crowsnest homes see zero behavior change.
...
Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional X mode, the Crowsnest, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/crowsnest.md](docs/crowsnest.md) - the opt-in two-way Google Chat bridge (the Crowsnest): reply model, components, runtime state, enabling, config keys, and verification.
- [docs/wake-resident.md](docs/wake-resident.md) - the opt-in wake-resident secondmate lifecycle: dormant by default, raised by a message through the ordinary spawn, stood down after a quiet stretch with its home and lease intact.
