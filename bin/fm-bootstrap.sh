...
#          (secondmate_sync, x_mode_setup, usage_mode_setup, fleet_sync) while still printing
...
usage_mode_setup() {
  ...
  if ! fm_usage_enabled; then
    if [ -e "$shim" ]; then
      if rm -f "$shim" 2>/dev/null && [ ! -e "$shim" ]; then
...
EOF
  ...
  if write_if_changed "$shim" "$shim_body" && chmod +x "$shim" 2>/dev/null; then
