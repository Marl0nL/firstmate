...
# Write a watcher check shim privately and bind it to the check trust path. The
# watcher REFUSES an unregistered check instead of running it, so arming a shim
# and registering it are one operation and never two. Registration is re-asserted
# even when the body is unchanged, so a shim written by an older firstmate is
# adopted on the next bootstrap rather than starting to fail silently.
write_and_register_check() {  # <state> <id> <body>
  local state=$1 id=$2 body=$3
  ( umask 077; write_if_changed "$state/$id.check.sh" "$body" ) || return 1
...
x_mode_setup() {
  ...
  x_mode_remove_artifacts() {
    ...
    fm_custom_check_trust_remove "$STATE" x-watch || true
    [ ! -e "$shim" ] && [ ! -e "$cadence" ] && [ ! -e "$STATE/x-watch.check-trust" ]
  }
...
EOF
  ...
  write_and_register_check "$STATE" x-watch "$shim_body" || { fmx_arm_failed; return 0; }
...
# idempotent, gitignored artifact:
#   state/usage-watch.check.sh - check shim that execs bin/fm-usage-poll.sh each
#                                cycle; the poll updates the ledger silently and
#                                prints a wake line ONLY when the quota severity
#                                first crosses a threshold (see docs/usage-monitor.md)
# On opt-out it removes the shim so the instance reverts to no automatic polling.
# Inert and silent when never opted in. The poll/report can still be run by hand
# regardless of this switch.
usage_mode_setup() {
  local shim shim_body tool missing
  shim="$STATE/usage-watch.check.sh"
  if ! fm_usage_enabled; then
    if [ -e "$shim" ]; then
      if rm -f "$shim" 2>/dev/null && [ ! -e "$shim" ] \
        && fm_custom_check_trust_remove "$STATE" usage-watch; then
        echo "USAGE: monitor off - removed the usage poll shim; restart the watcher (bin/fm-watch-arm.sh --restart) to stop polling"
      else
        echo "USAGE: monitor off - failed to remove the usage poll shim"
      fi
    fi
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    if [ -e "$shim" ]; then
      ...
      fm_custom_check_trust_remove "$STATE" usage-watch || true
    fi
    echo "USAGE: monitor off - missing poll dependencies; install them and rerun bootstrap"
    return 0
  fi
  mkdir -p "$STATE" 2>/dev/null || { echo "USAGE: monitor off - could not create state dir"; return 0; }
  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - token-usage poll shim.
# The watcher runs this each check cycle; the ledger update is silent and only a
# quota-severity threshold crossing prints a check: wake (see docs/usage-monitor.md).
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-usage-poll.sh")
EOF
)
  if write_and_register_check "$STATE" usage-watch "$shim_body"; then
    echo "USAGE: monitor on - usage poll armed via state/usage-watch.check.sh"
  else
    rm -f "$shim" 2>/dev/null
    fm_custom_check_trust_remove "$STATE" usage-watch || true
    echo "USAGE: monitor off - failed to arm the usage poll shim"
  fi
}
