...
#
# A home whose configured backlog backend is not tasks-axi (config/backlog-backend,
# owned by bin/fm-tasks-axi-lib.sh) has no backing store for a structured hold.
# There `hold` and `resolve` REFUSE rather than fake a durable record, while
# `complete` and `verify` degrade to the status-fold inventory: the attestation and
# the "every open decision is inventoried" gate still apply, no captain-held transfer
# line is written, and the status decision keeps carrying the item. Degraded runs say
# so on stdout. This keeps scout teardown gated instead of permanently refused.
set -eu
...
# backend is tasks-axi (config/backlog-backend=manual opts out, fm-tasks-axi-lib.sh
# owns that choice) AND the installed tasks-axi is compatible and exposes the
# captain-hold contract. A home that opted out has no backing store for a structured
# hold, so `hold` and `resolve` refuse rather than pretend, while `complete` and
# `verify` degrade to the status-fold inventory instead of bricking scout teardown.
hold_backend_available() {
  fm_tasks_axi_backend_available "$CONFIG" || return 1
  tasks-axi hold --help 2>&1 | grep -F -- '--kind captain' >/dev/null
}

require_hold_backend() {
  hold_backend_available || fail "durable captain holds require the tasks-axi backlog backend exposing --kind captain; this home has opted out (config/backlog-backend) or its tasks-axi is incompatible"
}
...
command_complete() {
  local origin=${1:-} meta previous='' supplied='' keys='' key status_file open raw_open key_seen=0 has_meta=0 durable=1
  [ "$#" -ge 2 ] || { usage >&2; exit 2; }
  validate_slug origin-id "$origin"
  shift
  meta="$STATE/$origin.meta"
  [ -f "$meta" ] && has_meta=1
  hold_backend_available || durable=0
  origin_exists_here "$origin" || fail "origin $origin is not owned by the active home $FM_HOME"
  if [ "$#" -eq 1 ] && [ "$1" = --none ]; then
    supplied=''
  else
...
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    # live status fold does not duplicate the same Captain's Call item. Only a
    # verified hold earns the transfer: without a backing store the status decision
    # MUST stay open, because the status fold is then the only surface carrying it.
    if [ "$durable" = 1 ]; then
      while IFS=$'\t' read -r key _verb _summary; do
        [ -n "$key" ] || continue
...
EOF
  ...
  printf 'complete: %s decision inventory reviewed%s%s\n' "$origin" "${keys:+ ($keys)}" \
    "$([ "$durable" = 1 ] || printf ' [durable backlog holds unavailable]')"
}

command_verify() {
  local origin=${1:-} meta reviewed keys key open durable=1
  [ "$#" -eq 1 ] || { usage >&2; exit 2; }
  validate_slug origin-id "$origin"
  meta="$STATE/$origin.meta"
...
$open
EOF
  printf 'verified: %s unresolved-decision inventory%s\n' "$origin" \
    "$([ "$durable" = 1 ] || printf ' [durable backlog holds unavailable]')"
}
