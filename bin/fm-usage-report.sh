...
#
# The report headlines real $ cost: 5-hour and weekly quota tiles + today's and
# lifetime cost; daily cost; and breakdowns by model, task, project, and harness
# ranked by real per-model pricing (fm_usage_pricing_json, sourced from claude-api
# - VERIFY the rates, they drift). A by_model row flagged "default rate" matched
# no rate-table entry and used the labelled fallback. The older token-class
# "weighted" number is kept alongside as a RELATIVE ranking heuristic only, not
# billing. The report labels itself "claude-harness usage" because non-claude
# crews are invisible here (see docs/usage-monitor.md).
set -u
...
# Effective per-model rate table (built-in defaults + optional config overlay);
# drives the real $ cost aggregated below via the shared pricing jq defs.
...
<div class="sub">claude-harness crews only &middot; $ cost is real per-model pricing (verify against current Anthropic pricing; rates drift) &middot; the token-class "weighted" number is a separate relative-ranking heuristic, not billing</div>
<div id="app"></div>
...
const usd = n => { n=n||0; if(n>=1000)return '$'+(n/1000).toFixed(1)+'k'; if(n>=1)return '$'+n.toFixed(2); if(n>0)return '$'+n.toFixed(4); return '$0'; };
...
// Bar tables rank by real $ cost. A by_model row carries priced:false when its
...
function barTable(title, rows, total){
  ...
  const body = rows.map(r=>{
    ...
    const tag = r.priced === false ? ' <span class="tag">default rate</span>' : '';
    return `<tr><td>${esc(r.key)}${tag}</td><td class="num">${fmt(r.requests)}</td><td class="num">${usd(r.cost)}</td>`
  ...
  return `<section><h2>${title}</h2><div class="barwrap"><table>`
    + `<thead><tr><th>${title.replace(/^By /,'')}</th><th class="num">reqs</th><th class="num">cost</th><th>share</th><th class="num"></th></tr></thead>`
...
function daysChart(days){
  ...
  return `<section><h2>Daily cost (last ${days.length}, USD)</h2><div class="days">${cols}</div></section>`;
}
...
tiles += tile('Today (cost)', usd(D.today_cost), fmt(D.totals.requests)+' requests all-time', 'normal');
tiles += tile('Total cost', usd(D.totals.cost), 'lifetime &middot; real per-model pricing', 'normal');
...
html += barTable('By model', D.by_model, t.cost);
html += barTable('By task', D.by_task, t.cost);
html += barTable('By project', D.by_project, t.cost);
html += barTable('By harness', D.by_harness, t.cost);
html += `<section><h2>Token-class split (raw tokens vs ranking weight)</h2><div class="sub">ranking heuristic only; see the real $ cost above</div><div class="barwrap"><table><thead><tr><th>class</th><th class="num">raw tokens</th><th class="num">weighted</th><th class="num"></th></tr></thead><tbody>${classBody}</tbody></table></div></section>`;
document.getElementById('app').innerHTML = html;
document.getElementById('foot').textContent = 'generated ' + (D.generated_at||'') + ' · quota source: ' + (q.source||'none') + ' · cost: real per-model pricing (verify rates)';
</script>
</body>
</html>
HTML_TAIL
