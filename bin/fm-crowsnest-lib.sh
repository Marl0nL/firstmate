...
# The shim helpers register through the check trust path (bin/fm-check-register.sh
# owns registration), and fm-crowsnest.sh reads registration state via
# fm_custom_check_registered. That reader calls fm-pr-lib.sh helpers
# (fm_pr_task_id_valid, fm_pr_file_device, fm_pr_private_file_valid), so pull in
# both libraries relative to this file. Guarded, so a caller that already sourced
# them (and a standalone `. bin/fm-crowsnest-lib.sh` in a test) both work.
if ! declare -F fm_pr_task_id_valid >/dev/null 2>&1; then
  # shellcheck source=bin/fm-pr-lib.sh disable=SC1091
  . "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fm-pr-lib.sh"
fi
if ! declare -F fm_custom_check_registered >/dev/null 2>&1; then
...
EOF
  ...
  # fm-check-register.sh requires the check shim at mode 700 (it is executed by
  # the watcher and validated as a private executable), so mark it executable
  # before registering its trusted bytes.
  chmod 700 "$shim" 2>/dev/null || return 1
  # Register the shim's trusted bytes; its "registered: ..." success line is
  # suppressed so crowsnest_setup owns the single CROWSNEST digest line, while
  # its exit status still gates wiring success.
  "$FM_ROOT/bin/fm-check-register.sh" chat-watch >/dev/null
}
...
fmc_unwire_shim() {
  ...
  rm -f "$state/chat-watch.check-trust" 2>/dev/null || true
