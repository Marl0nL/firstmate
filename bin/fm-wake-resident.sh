...
cmd_sync() {
  ...
  fm_wr_wire_main_shim "$FM_ROOT" "$FM_HOME" "$STATE" \
  ...
  while IFS= read -r name; do
    ...
    fm_wr_wire_self_shim "$FM_ROOT" "$home" "$inbox" \
...
cmd_standdown() {
  ...
  while [ "$waited" -lt "$STANDDOWN_TIMEOUT" ]; do
    ...
    if fm_backend_agent_confirmed_absent "$backend" "$target" 2>/dev/null; then
      break
    fi
    sleep 1
    waited=$((waited + 1))
  ...
  if fm_backend_target_exists "$backend" "$target" 2>/dev/null \
     && ! fm_backend_agent_confirmed_absent "$backend" "$target" 2>/dev/null; then
    rm -f "$snapshot"
