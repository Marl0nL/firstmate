...
# a registered process-event source (state/procevent/*.source), a non-empty
# durable wake queue (state/.wake-queue) still waiting to be delivered, or an
...
fm_sup_network_stage_active() {
  local state=$1 status_file stage pid started budget age
  status_file="$state/.startup-network.status"
  [ -f "$status_file" ] && [ ! -L "$status_file" ] || return 1
  stage=$(sed -n 's/^state=//p' "$status_file" 2>/dev/null | tail -1)
  [ "$stage" = running ] || return 1
  pid=$(sed -n 's/^pid=//p' "$status_file" 2>/dev/null | tail -1)
  case "$pid" in ''|*[!0-9]*|0) return 1 ;; esac
  kill -0 "$pid" 2>/dev/null || return 1
  started=$(sed -n 's/^started=//p' "$status_file" 2>/dev/null | tail -1)
...
#   FM_SUP_QUEUE_PENDING  true/false - state/.wake-queue has undelivered records
