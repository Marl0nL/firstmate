...
# A terminal record (done/timeout/failed) still reads true for 15s after its
# `finished` stamp, because that script writes the terminal state
# BEFORE appending the wake: without the bridge, a status sampled between those
# two steps reads neither a running stage nor a pending queue, and the wake lands
# in a home that just decided it needed no supervision. A .startup-network
# .delivered marker ends the bridge immediately, since an inline harvest already
# took the result and no wake is coming.
