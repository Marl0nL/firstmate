...
#
# Captain's Call is the UNION of two authoritative sources: the canonical snapshot's
# hints.open_decisions status fold (fm-classify-lib.sh owns the durable keyed-decision
# contract) and structured captain-held backlog items (bin/fm-decision-hold.sh). The
# union is what keeps a LIVE crew's needs-decision:/blocked: visible in the window
# before it has a durable owner, while the durable hold carries it past the crew's
# teardown; the `captain-held` transfer verb closes the status copy, so a decision
# with both sources renders once, not twice. It never infers decisions from report or
# visual-review prose.
#
