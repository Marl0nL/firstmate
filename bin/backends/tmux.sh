...
fm_backend_tmux_create_task() {  # <session> <window-name> <proj-abs> -> prints window id
  ...
  if tmux list-windows -t "$ses" -F '#{window_name}' | grep -qx "$wname"; then
    echo "error: window $ses:$wname already exists. Verify nothing live is running in it first (tmux capture-pane -p -t '=$ses:=$wname' shows what its pane is doing); only if it is not a live agent, remove it and retry: tmux kill-window -t '=$ses:=$wname'" >&2
    return 1
  fi
