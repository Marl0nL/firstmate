...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real, correctly launched agent process.
  Investigate the reason because that secondmate is not guaranteed live and working.
  A manual-permission-mode reason means the mate is live but Herdr restored it without firstmate's launch flags, so it will freeze on its first non-allowlisted command; resolve the in-flight work or failed exit the line names, then recover it with the same guarded exit-then-`--secondmate`-respawn cycle the sweep uses (`docs/herdr-backend.md` "Restored-manual-mode secondmate stall").
