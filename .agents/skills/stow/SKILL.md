...
3. **Write within firstmate's existing write boundaries.**
   ...
   - Task-scoped notes: inspect the relevant backlog item with `tasks-axi show --file "$FM_HOME/data/backlog.md" <id> --full`, judge whether the new note is new, duplicate, superseding, or obsolete, then write a considered replacement body with `tasks-axi update --file "$FM_HOME/data/backlog.md" <id> --body-file <path>`.
