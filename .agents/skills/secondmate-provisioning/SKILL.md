...
A present primary value always converges byte-exact into validated secondmate homes, and primary absence removes the destination so those homes keep runtime auto-detection - except that when the primary's effective backend is herdr (whether set or auto-detected), convergence pins `config/backend=herdr` downstream through the inheritance effective-backend override in [`fm-config-inherit-lib.sh`](../../../bin/fm-config-inherit-lib.sh), because a claim-suppressed (`HERDR_ENV=0`) secondmate supervisor pane can no longer auto-detect herdr itself.
