...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it launches the watcher detached from its own process tree, verifies it is genuinely alive with a fresh liveness beacon, and prints exactly one honest status line (`started` / `attached` / restart-only `healthy` / `FAILED`, the last exiting non-zero).
The arm stays the harness-visible waiter for that whole cycle, but the watcher is deliberately neither its descendant nor a member of its process group, because stopping a harness background task kills the launching shell's process tree; the arm's header owns that detach contract and `docs/turnend-guard.md` records the measurements behind it.
...
The watcher SELF-RENEWS: on an actionable-wake exit it releases the singleton and hands off to a detached successor, so detection survives a re-arm the agent never sends, while delivery still rides the arm's exit.
Renewal is suppressed while `state/.afk` exists, because the away-mode daemon owns triage and needs a one-shot watcher.
Because a live watcher can therefore outlive the arm that would deliver its wakes, the arm records itself in `state/.watch-waiter` for as long as it waits, and the guards check that delivery half too.
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if tasks are in flight and the watcher is not running, is running with no arm waiting on it, or queued wakes are waiting to be drained.
`bin/fm-supervision-lib.sh` is the one owner of the liveness verdict and answers it from the singleton lock, never the beacon alone; a 2026-07-31 incident in which those two answers disagreed, and the beacon-based one stayed silent, is recorded in [turnend-guard.md](turnend-guard.md).
...
If this session did not get the fleet lock, both surfaces keep the alarm but switch to read-only wording with no repair command.
`bin/fm-lock.sh`'s header owns its two distinct refusals - another live session holds the lock, versus this session cannot identify its own harness process - and session start names which one happened, because only the first means a competing session exists.
