...
That row produces a notification only once it has stayed the oldest one with no acknowledgement progress for a threshold derived from the doorbell re-ring ladder (`FM_TASK_INBOX_GRACE_SECS` x `FM_TASK_INBOX_RING_MAX`, floored at 60 seconds, overridable verbatim by `FM_SECONDMATE_WAKE_STALL_SECS`), and only while the mate's pane is neither provably busy nor in a declared paused or captain-held state; a poll suppressed by either gate re-anchors the freeze clock, so freeze time accrues only while the mate is idle and expected to be draining, and a healthy resume from a pause or a long turn starts a fresh threshold window instead of firing immediately.
A terminal verb (done, failed, blocked, needs-decision) is deliberately not a suppressing state, because on a persistent secondmate it is a routine report line in its reply log rather than a lifecycle stop, so a wake loop wedged behind a months-old `done:` or `failed:` line still surfaces; a paused or captain-held wait is suppressed instead because it carries its own independent bound through the pause-resurface cadence, and the captain's stated priority accepts a false alarm on a legitimately-stopped mate over a missed one.
A healthy mate draining its own queue advances the oldest row every turn, so its freeze clock resets and never accumulates and the raw enqueue age of any single row is never the trigger; the primary appends its one keyed `check` wake naming the mate, row sequence, and frozen duration only for a genuinely wedged loop, with parent receipts and queued-key deduplication suppressing repeats for the same row across watcher and handling crashes, while empty, advancing, busy, declared-wait, and still-draining queues remain silent.
...
`tests/fm-wake-queue.test.sh` pins the notification, idempotence, quiet-queue, freeze-anchoring, health-gate, derived-threshold, and byte-for-byte foreign-row preservation guarantees.
...
Claude's `bin/fm-claude-stop-autoarm.sh` hook fires on every Stop and, when the home is eligible and still needs supervision, claims one home-scoped cycle, runs the arm wrapper as a waited child of the hook, and translates actionable closes into exit-2 rewakes.
...
Its alarm trigger set is deliberately narrower than the turn-end one below: a pending queued wake and an in-progress deferred network stage are arming need only, because session start enqueues and launches them itself and then runs this mid-turn guard before the turn's Stop has armed any watcher.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, a process-event source, Relay polling, a pending durable wake queue, or an in-progress deferred network stage needs supervision and no identity-matched watcher lock with a fresh beacon is live, blocking-capable Stop hooks block and nonblocking turn-end integrations force one bounded follow-up.
...
For compatibility, default tmux tasks do not write `backend=tmux`; every reader treats a missing `backend=` field as `tmux`, except a record carrying `remote_host=`, which classifies as the non-local `remote` placement instead.
That placement has no local runtime adapter, so every local dispatch - capture, send, kill, composer and agent state - refuses it (agent state degrades to `unverified`, never to a false `dead`), while the local endpoint-existence probe reports it present because only the remote transport can judge a remotely placed second mate's liveness; [`remote-secondmates.md`](remote-secondmates.md#normal-operation) owns how those reads route to its host.
