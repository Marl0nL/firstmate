...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, Relay polling, or an ackable queued wake has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
Its trigger set is deliberately narrower than the turn-end one below: an in-progress deferred network stage is arming need only, because session start launches that stage itself and then runs this mid-turn guard before the turn's Stop has armed any watcher.
