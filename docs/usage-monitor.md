...
- `severity-watermark` - the last surfaced CONFIRMED alert level, so the wake fires only on an upward crossing and re-arms when the level eases.
- `severity-history` - the append-only, inspectable trail of recent quota-severity samples the wake decision is debounced against; one TSV line per sample (`epoch, iso, raw level, confirmed level, emitted`), aged out past a bounded cap. Never a single overwritten value, so a caller can see WHY a wake did or did not fire.
- `.last-poll` - mtime marker for the `--if-due` min-interval gate (touched only after a completed scan); `.poll.lock` is the single-writer lock.
...
2. **Wake-drain (the decoupled, always-hit trigger).** `bin/fm-wake-drain.sh` runs at the top of every wake-handling turn regardless of watcher liveness, and fires `bin/fm-usage-poll.sh --if-due`.
   `--if-due` is self-gated: it no-ops unless the monitor is opted in AND the last completed `--if-due` scan marked `data/usage/.last-poll` more than `FM_USAGE_POLL_MIN_INTERVAL` seconds ago (default 60), and it relies on the same offset/mtime checkpoint so it only reads new transcript bytes.
   The marker is advanced only after a scan pass completes under the single-writer lock, so a run that lost the lock and scanned nothing never suppresses the next catch-up.
   To keep supervision correctness first, the wake-drain call skips the fork entirely unless the opt-in check-shim exists (a cheap pre-filter; the poll still self-gates), then runs the poll DETACHED in the background - so a slow scan can never block the drain or the wake-queue lock it still holds, on any platform, not only where `timeout` exists - with `timeout` layered on as a secondary bound where available.
   It implies `--quiet` (never emits a wake) and is swallowed with `|| true`; unlike the watcher, a reaped or slow poll is harmless because the ledger is idempotent and single-writer-locked.
...
- The check shim `state/usage-watch.check.sh` (generated by bootstrap on opt-in, mirroring X mode's `x-watch.check.sh`) execs `bin/fm-usage-poll.sh` each cycle. Bootstrap also registers it through the watcher's check trust path in the same operation, and re-asserts that registration on every run, so a shim written by an older firstmate is adopted rather than starting to fail; the watcher refuses an unregistered check instead of running it. If you ever see `check: rejected unauthenticated state checks: ... usage-watch.check.sh`, read the file and re-bind it with `bin/fm-check-register.sh usage-watch` (or just re-run bootstrap). See [Watcher checks are authenticated](architecture.md#watcher-checks-are-authenticated). The ledger update is a silent side effect; the poll prints a wake line only when the quota severity first crosses into warning/critical AND that severity is CONFIRMED across consecutive samples. This is the only path that can emit the quota wake, and it fires only while the watcher is alive.

The wake DECISION is debounced (the SOURCE is left untouched, and nothing about what counts as critical is weakened). The `/api/oauth/usage` read is undocumented/best-effort and degrades to a burn-rate estimate, so it RARELY emits a lone sample that maps to a high severity between otherwise-normal reads; waking on any single such sample cried wolf 9 times in one away period while the session window sat at ~1% (see the 2026-08-03 learning). `fm_usage_record_severity` (in `bin/fm-usage-lib.sh`) instead requires a severity to hold across the last `FM_USAGE_ALARM_CONFIRM` samples (default 2, clamped to a floor of 2 - the "two bracketing reads" rule) before it surfaces: the confirmed level is the MIN of the last N raw samples, so a lone extreme is bracketed by a normal read and never confirms, while a genuine sustained crossing still alarms on the Nth consecutive sample. The `severity-watermark` then tracks that last-surfaced CONFIRMED level, so a sustained level surfaces once and a drop back down re-arms the alarm for a later genuine crossing. Every sample is appended to `severity-history`, so the recent trail is inspectable rather than a single self-resetting value. Only the watcher check cycle records a sample; the `--if-due` and `--backfill --quiet` catch-ups are silent and never touch the debounce state.
...
- Wake-drain runs `--if-due` (detached, gated on the opt-in check-shim) on every wake-handling turn to keep the ledger current even when the watcher is dead; it is silent, self-gated, and failure-isolated, and never emits a wake.
