...
When work, a process-event source, Relay polling, a pending durable wake queue, or an in-progress deferred network stage needs supervision at that boundary and no identity-matched watcher has a fresh beacon, the harness integration must either block the turn end or force one bounded follow-up that uses the recovery instruction from the emitted session-start protocol.
...
Both are arming need only: `bin/fm-guard.sh` deliberately leaves the pending wake queue and the in-progress network stage out of the mid-turn watcher-down alarm, because session start enqueues and launches them itself and then runs the pull warning before the turn's Stop has armed any watcher, so alarming there would report an ordinary startup shape as a supervision lapse.
...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  Claude Code enforces that declared timeout by SIGTERMing the hook's whole process group, watcher included, and no unbounded value exists: an absent `timeout` falls back to Claude's own 600-second default, and any declared value schedules the kill ([`verification/supervision.md`](verification/supervision.md#turn-end-guard) records the 2026-09-02 evidence on Claude 2.1.258).
  Raising the number therefore only moves the blind cliff, so the auto-arm renews continuity before it: shortly before the deadline (`FM_CLAUDE_AUTOARM_RENEWAL_BUDGET`, default 27000 seconds, safely below the declared 28800) it closes its own arm under the pid-matched renewal marker - recorded in the lifecycle ledger as `continuity-renewal` with a successor disposition, never as `arm-interrupted` with none - and exits 2 with a benign renewal banner, so one bounded turn ends, the next Stop fires, and a fresh firing arms with a fresh timeout budget.
  The renewal re-checks the away, supervision-need, live-watcher, and post-alarm gates before rewaking, records `outcome=renewal` as owned recovery, and its worst-case unsupervised gap is that one bounded turn, with the durable wake queue preserving any event that lands inside it.
  A renewal close that leaves another verified live watcher with a fresh beacon - the attached-arm case, where the TERM closes only the arm - costs no turn at all: it takes the same silent `clean` benign close as every other healthy cycle end.
  Because the whole mechanism depends on the budget staying below the declared timeout, the hook refuses any `FM_CLAUDE_AUTOARM_RENEWAL_BUDGET` that reaches its `28800` ceiling or is not a positive number of seconds - including a leading-zeros value that normalizes to zero, which would renew the instant the arm starts - prints one stderr line naming the refusal from the firing that actually arms (every firing that stays inert stays silent), and uses the safe default instead.
  That default is derived from the ceiling rather than written beside it, so the ceiling binds unconfigured homes too and lowering the declared timeout lowers every home's budget with it.
  `tests/fm-claude-stop-autoarm.test.sh` pins both halves against the tracked registration's declared timeout - the refused ceiling and the budget the refusal actually falls back to - so the two files cannot drift apart silently.
...
The Claude mode waits up to `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds) and allows the stop when the watcher is healthy, `state/.claude-autoarm.lock` has a live `autoarm` role owner whose supervision decision is still open and whose eventual failure must exit 2, or `state/.claude-autoarm-epoch` contains a fresh actionable rewake or declared-timeout renewal owned by this event epoch.
A fresh `renewal` outcome is owned recovery exactly like a fresh `rewake`: the closed cycle already owns one benign continuation and the next Stop-owned firing re-arms, so the guard never consumes a blocked-stop count for it.
...
An `arming` entry stays in flight however old it is, because the owner waits on its arm child for the whole watcher cycle.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, including the pending-wake-queue and deferred-network-stage arming cases, the unackable-only queue and the crashed or over-deadline stage record that stay inert, the bounded terminal-state grace across the publish-then-enqueue gap, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, the fresh and stale renewal-epoch decisions, the abandoned auto-arm claim cases that must block or clear instead of allowing a blind stop, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
`tests/fm-claude-stop-autoarm.test.sh` covers the declared-timeout renewal itself: the deadline handoff with its benign banner and untouched block budget, the zero-turn silent close taken instead when a verified live watcher survives the renewal, the same handoff in a marked secondmate home with no per-home configuration, inertness under away mode, a home gone idle, a non-owner session, and the attended fail-open, and - with the real arm wrapper at the timeout boundary - the `continuity-renewal` lifecycle classification with a successor disposition and no orphan watcher.
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the narrower alarm need that exempts a running network stage and a pending queued wake from the mid-turn banner, the unackable-only queue that advises no drain, the persistent-model fresh-leftover-beacon negative control, the auto-arm model's healthy fresh-beacon-without-a-watcher case and stale-beacon alarm, and the extension model's live-watcher path, ownership-qualified fresh hand-off, held-lock failures, independently broken ownership signals, stale-beacon alarm, queued-wake warning, and Pi and pi-signed harness routing.
...
[`verification/supervision.md`](verification/supervision.md#turn-end-guard) records the active cross-harness empirical evidence, including the 2026-07-24 Claude `asyncRewake` revalidation and the 2026-09-02 declared-timeout-bound and renewal verification.
