...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  ...
  Because the whole mechanism depends on the budget staying below the declared timeout, the hook refuses any `FM_CLAUDE_AUTOARM_RENEWAL_BUDGET` that reaches its `28800` ceiling, prints one stderr line naming the refusal from the firing that actually arms (every firing that stays inert stays silent), and uses the safe default instead, and `tests/fm-claude-stop-autoarm.test.sh` pins the tracked registration's declared timeout against that ceiling so the two files cannot drift apart silently.
