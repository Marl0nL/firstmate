...
`tests/fm-claude-stop-autoarm.test.sh` covers the declared-timeout renewal itself: the deadline handoff with its benign banner and untouched block budget, inertness under away mode, a home gone idle, a non-owner session, and the attended fail-open, and - with the real arm wrapper at the timeout boundary - the `continuity-renewal` lifecycle classification with a successor disposition and no orphan watcher.
