...
| `fm-check-lib.sh`        | Shared watcher check-script authentication: trust records, private-artifact rules, verified snapshots |
| `fm-check-register.sh`   | Bind a `state/<id>.check.sh` to its exact bytes so the watcher will run it; list and verify checks |
...
| `fm-pr-check.sh`         | Record `pr=` and `pr_head=` for a PR-ready task, then arm and register the watcher's merge poll |
...
| `fm-crowsnest.sh`        | Crowsnest operator lifecycle CLI: enable, disable, register, unregister, autostart, status |
| `fm-crowsnest-lib.sh`    | Shared Crowsnest config resolution, inbox helpers, and check-shim wiring             |
| `fm-crowsnest-relay.sh`  | The registered `firstmate` agent: stash a Chat message, enqueue a `chat-mention` wake, ack immediately |
| `fm-crowsnest-poll.sh`   | The watcher check shim: surface the oldest pending inbox entry as `chat-mention <id>` |
| `fm-crowsnest-post.sh`   | Post a Crowsnest reply into a pending message's thread, or a proactive space/thread post |
| `fm-crowsnest-post.py`   | Transport half of the post tool: post one message through the backend's own `ChatClient` |
