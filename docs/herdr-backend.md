...
Closing its last tab can remove the workspace, and the next spawn recreates it; whether Herdr accepts that last-tab close is release dependent, so a deliberate workspace removal uses `workspace close` rather than a last-tab `tab close` ([`verification/runtime-backends.md`](verification/runtime-backends.md#last-tab-teardown-guard)).
...
`fm_backend_agent_launch_health` (`bin/fm-backend.sh`, backed by the pure classifier in `bin/fm-launch-health-lib.sh`) refines an `alive` Claude verdict: it reads the foreground process's argv from `pane process-info` for `--dangerously-skip-permissions`.
A live Claude carrying the flag is `healthy`; one provably lacking it is `degraded`; an empty or unreadable argv is `unknown` and preserves the pane, so a healthy firstmate-launched mate is never cycled.
The permission flag rides argv and is the verdict's sole authority: a firstmate launch always carries it in argv, and the `HERDR_ENV=0` / `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false` env prefixes `bin/fm-spawn.sh` also sets are launch details, deliberately not detection signals (whether a Herdr resume preserves the original process environment is unverified, so an environ-based read could silently mask a degraded husk).
...
Both layers are deliberately local to the captain's host: the sweep's remote `alive` branch does not consult launch health and the config editor edits only the local global config, so covering a remote host's own restart (where that host's Herdr resumes the mate flag-less) is a decided, separately tracked follow-up rather than an oversight.
...
`fm_backend_herdr_pane_foreground_beyond_shell` (is any foreground process something other than the pane's own shell, per the `pid` against `shell_pid` that already proves a pane is at an idle prompt) is the fourth, and answers the only identity question a deliberately non-harness command can pass: it exists for `bin/fm-autostart.sh`'s `-- <argv>` escape hatch, where the harness probe can never answer yes while a bare shell must still read as a husk.
`bin/fm-autostart.sh` is the first such caller: its "a firstmate is already running" guard reads the union of `agent list` and `pane list` and then requires every matching entry to be confirmed by `pane process-info`, so neither a post-reboot ghost record nor a registry that cannot see a live Claude can make the boot unit skip starting the real one, and a restored bare shell in the firstmate home cannot make it skip starting one either.
Which of the two identity probes that confirmation uses is chosen from the argv the run was asked to launch, so the default harness launch keeps the strict verified-harness requirement while a custom `-- <argv>` command is confirmed by holding the pane beyond its own shell.
The dated reboot-ghost evidence, and that a live `agent start` claude reported `agent_status: "unknown"` on Herdr 0.7.4, are recorded under [`verification/runtime-backends.md`](verification/runtime-backends.md#herdr); the boot path's own launch and presence surfaces are re-measured against the real binary by `FM_AUTOSTART_HERDR_LIVE=1 tests/fm-autostart-herdr-live-e2e.test.sh`, recorded there under "Boot autostart launch shape".
...
It provisions only non-default names beginning with `fm-lab-`, supplies an explicit leading `--session` on allowed task commands so the flag is parsed as a global option and cannot be captured by an argv after a `--` separator (notably `agent start`), refuses caller-supplied session flags and server/session lifecycle subcommands, and performs destructive stop/delete only through its guarded lifecycle actions.
