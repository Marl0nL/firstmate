...
So on a re-emit `bin/fm-session-start.sh` leads with an `OPERATIONAL MEMORY` section carrying those files, right after the wake queue and ahead of the bulky fleet state, and the `CONTEXT` section then only points back to it; a true startup keeps memory in `CONTEXT`.
...
That re-emit-only section is its own `operational-memory` runtime-bound stage, so a re-emit truncated while printing captain or learnings names it rather than blaming the wake queue and sending the agent to `CONTEXT`, which on a re-emit no longer carries the memory.
...
They also prove the directive names a concrete status file only for the canonical `<home>/state/<id>.inbox` shape, whose sibling `<id>.status` is the channel the parent actually reads, and falls back to charter-relative phrasing for a routed inbox such as the remote leg's `state/parent-route/`, where the derived sibling is a file nothing mirrors.
