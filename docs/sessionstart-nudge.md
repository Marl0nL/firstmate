...
`bin/fm-session-start.sh`'s header owns that ordering; this section owns why the delivery budget forces it.
...
It also proves a `clear` re-emit leads with this home's memory ahead of the bulky fleet state and within the inline preview budget, printed once, while a true startup keeps that memory after the fleet state.
`tests/fm-task-inbox.test.sh` and `tests/fm-send-secondmate-marker.test.sh` prove the marked-request doorbell's correlated-reply directive: it names the corr token and the derived status file, stays a single line, is generic so every marked record in one inbox rings an identical drain-all doorbell, and never touches an unmarked steer's doorbell.
