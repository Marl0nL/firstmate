...
## Re-emit delivery budget

A run-tier session-open channel does not deliver an unbounded payload into model context.
Claude saves hook output over about 10,000 characters to a file and hands the agent only a HEAD preview plus that file path (some builds have been observed truncating lower, near 2,000); Pi retains a bounded prefix of the injected digest and appends a loud truncation marker; both keep the HEAD and drop or defer the tail.
A `clear` or `compact` re-emit routinely exceeds that budget - a working home's fleet state alone can, and a populated home's whole digest far exceeds it - so whatever a re-emit orders last is exactly what a freshly cleared agent, acting on the preview, never sees.
The digest's default ordering puts the durable memory files last so a tail-truncating startup delivery drops the recoverable thing first (see the ORDERING note in `bin/fm-session-start.sh`), but that is the wrong end for a re-emit that lost precisely that memory: a `/clear` discards the charter and prior context, and `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md` reach the agent only through this digest, while `AGENTS.md` is re-injected by the harness every turn.
So on a re-emit `bin/fm-session-start.sh` leads with an `OPERATIONAL MEMORY` section carrying those files, right after bootstrap and ahead of both the wake queue and the bulky fleet state, and the `CONTEXT` section then only points back to it; a true startup keeps memory in `CONTEXT`.
Leading is necessary but not sufficient: `config/startup-memory-budget` bounds those files in estimated tokens, so a home can hold enough sanctioned memory for one file to fill the whole preview and push the fleet state, the read-once contract, and the rest of the memory back out of it.
So that section also bounds each file, and a file that exceeds its bound prints its head plus one pointer naming its full path and byte size; the `CONTEXT` path on a true startup stays unbounded.
Bounding the memory only makes the budget hold if what precedes it is bounded too, and the wake queue is not: `bin/fm-wake-drain.sh` caps `OPEN DECISIONS` and `RECORD DIVERGENCE` individually but prints the wake rows and unread status tails unbounded, and open decisions are fleet-wide and scanned on every drain including the empty-queue fast path.
That is why memory leads the wake queue rather than merely the fleet state: only the fixed lock/bootstrap preamble runs ahead of it, so the per-file bound is a guarantee for a busy home and not only a quiet one.
The queue loses nothing by following it - it is durable, stays unacknowledged until `WAKE_ACK_REQUIRED` runs, and is re-drained at the start of every wake-handling turn (AGENTS.md section 8) - whereas a memory delivery cut out of the preview has no second chance.
That re-emit-only section is its own `operational-memory` runtime-bound stage, so a re-emit truncated while printing captain or learnings names it rather than blaming an adjacent stage and sending the agent to `CONTEXT`, which on a re-emit no longer carries the memory.
`bin/fm-session-start.sh`'s header owns that ordering and the bound; this section owns why the delivery budget forces them.
The correlated-reply directive `bin/fm-task-inbox-lib.sh` appends to a marked-request doorbell is the harness-independent backstop for the one case a truncated preview still cannot be trusted to carry, and it reaches nudge-tier and uncovered surfaces this re-emit ordering cannot.
...
It also proves a `clear` re-emit leads with this home's memory ahead of both the wake queue and the bulky fleet state and within the inline preview budget, printed once, that an oversized memory file is cut there with a pointer to its full path while a small one prints whole, and that a true startup keeps that memory after the fleet state and uncut.
`tests/fm-task-inbox.test.sh` and `tests/fm-send-secondmate-marker.test.sh` prove the marked-request doorbell's correlated-reply directive: it names the corr token, stays a single line, is generic so every marked record in one inbox rings an identical drain-all doorbell, and never touches an unmarked steer's doorbell.
They also prove the directive names the channel the parent really reads for each inbox shape this library owns, never a derived path that happens to be adjacent: the canonical `<home>/state/<id>.inbox` gets its sibling `<id>.status`, while the remote steer leg's `state/parent-route/<id>.inbox` gets that home's fixed relay log `state/parent-replies.status` rather than the `parent-route/<id>.status` nothing mirrors.
An unrecognized sub-plane guesses no path and falls back to charter-relative phrasing.
...
It also proves the stage list follows the path actually run: a truncated re-emit lists `operational-memory` for reconciliation, and a truncated startup, which never emits that section, does not.
