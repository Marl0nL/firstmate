...
Domain-local preferences for one captain's fleet live locally in each home's `data/captain.md`; it is gitignored and printed in the session-start digest with the other memory files, in the position the `bin/fm-session-start.sh` header owns for the path being run.
...
Fleet-local operational facts and gotchas live locally in `data/learnings.md`; it is gitignored and printed after the captain-preference files in the session-start digest.
...
The allowance bounds how much memory a home may hold, not how much every delivery carries: a `clear` or `compact` re-emit additionally bounds each of the three files to fit the session-open inline budget and names the full path of any file it cut ([re-emit delivery budget](sessionstart-nudge.md#re-emit-delivery-budget)).
...
The registry's charter-summary field is always deterministically truncated to a short one-liner, since the whole registry prints in full at every session start; set `FM_SECONDMATE_SUMMARY` to supply that short summary directly instead of deriving it from the (possibly huge) charter text.
The scope field is never truncated because it is the routing input the main firstmate matches work against at intake.
...
A dormant wake-resident home or a confirmed bare shell is never sent this nudge on either the fresh or retry path; bootstrap reports the skip as `BOOTSTRAP_INFO:`, and [docs/wake-resident.md](wake-resident.md#the-reread-nudge-exemption) owns that exemption.
...
FM_TASK_INBOX_GRACE_SECS=90   # seconds an unhandled steering-inbox message may sit before the watcher attempts doorbell delivery on an idle pane; also the minimum spacing between attempts; also a factor of the derived FM_SECONDMATE_WAKE_STALL_SECS default below
FM_TASK_INBOX_RING_MAX=3      # watcher delivery attempts without an acknowledgement before the task surfaces as a stale wake for recovery; also a factor of the derived FM_SECONDMATE_WAKE_STALL_SECS default below
...
FM_CLAUDE_AUTOARM_RENEWAL_BUDGET=27000   # seconds a Claude Stop auto-arm may hold one arm open before renewing continuity; must stay below the declared hook timeout (28800) that kills the whole hook process group, and a value reaching that ceiling - or any non-positive-integer value - is refused on stderr and replaced by this default
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh watcher before reporting FAILED, and the same bound it waits out an in-flight Claude Stop auto-arm continuity renewal before starting a watcher normally; default 30 on Git Bash/MSYS
...
FM_SECONDMATE_WAKE_STALL_SECS=     # minimum FROZEN duration (the oldest valid foreign wake-queue row staying oldest with no ack progress) before an endpoint-recorded local secondmate whose pane is not provably busy and not in a declared pause/captain-held state produces one durable parent wake-loop-stall notification; a terminal verb (done/failed/blocked/needs-decision) deliberately does NOT suppress, because on a persistent secondmate it is a routine report line rather than a lifecycle stop, so a wedge behind a months-old done:/failed: line still surfaces (a false alarm on a legitimately-stopped mate is the accepted trade against a missed alarm); unset, zero, or invalid derive it from the doorbell re-ring ladder as FM_TASK_INBOX_GRACE_SECS x FM_TASK_INBOX_RING_MAX (default 90 x 3 = 270), the point past which a healthy mate's own doorbell would have exhausted re-delivery, with the derived value floored at 60 so a zero ladder cannot make every observed row an instant stall (an explicit positive override is used verbatim, never floored); a provably busy or declared paused/captain-held poll re-anchors the freeze clock rather than merely muting the wake, so a resume must stay frozen for a fresh full threshold before firing; row age alone is deliberately NOT the trigger, since a healthy mate draining its own queue leaves each successive oldest row aged
