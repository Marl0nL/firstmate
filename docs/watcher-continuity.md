...
It also covers the atomic-admission ownership gate across both bounded mid-iteration spans - a watcher superseded during the signal grace, and one superseded during the triage probe that follows it, each stand down without enqueuing, absorbing, or advancing the seen-marker for a signal they no longer own - the `fm_autoarm_renewal_in_progress` predicate over live, dead, absent, and malformed markers, and the arm refusing to fork a competing watcher during a live renewal while starting normally once it clears.
