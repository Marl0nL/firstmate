...
The watcher's own singleton admission contract - atomic acquisition, the settle re-check that brackets the mid-renewal window, and the ownership predicate that gates every multi-second supervision side-effect path (the signal batch at the loop top, after the grace linger, and after the triage probe; the check block around each network poll; the pane-stale loop around each window's capture) so a superseded watcher does no work - is stated once in the `bin/fm-watch.sh` header.
Every one of those gates re-checks ownership before surfacing (`wake` / `fm_wake_append`) or suppressing (`.seen-*` / `.stale-*` marker writes), so a stood-down watcher leaves each signal, check result, and stale pane unmarked for the rightful holder to classify.
...
It also covers the atomic-admission ownership gate across the bounded mid-iteration spans - a watcher superseded during the signal grace, one superseded during the triage probe that follows it, and one superseded during a pane capture, each standing down without enqueuing, absorbing, advancing the seen-marker, or writing the stale suppressor for work they no longer own - the `fm_autoarm_renewal_in_progress` predicate over live, dead, absent, and malformed markers, and the arm refusing to fork a competing watcher during a live renewal while starting normally once it clears.
