...
The watcher's own singleton admission contract - atomic acquisition, the settle re-check that brackets the mid-renewal window, and the invariant that a superseded watcher performs no side effect - is stated once in the `bin/fm-watch.sh` header.
That invariant is category-wide, not a list of particular spans: `watcher_owns_lock` is re-checked immediately before every side-effect commit in the poll iteration - every surfaced wake and durable enqueue, every seen/hash/count/stale/stale-since marker write, every crew keystroke or doorbell send, and every herdr agent-state publish - and the watcher stands down the moment it no longer holds the singleton lock.
Capture and classify probes may run on a lost lock; no commit may follow one, so a stood-down watcher leaves each signal, check result, and stale pane unmarked for the rightful holder to classify.
...
It also covers the atomic-admission ownership gate across the bounded mid-iteration spans - a watcher superseded during the signal grace, one superseded during the triage probe that follows it, one superseded during a pane capture, and one superseded during a stale classify probe, each standing down without enqueuing, absorbing, advancing the seen-marker, or writing the stale suppressor for work they no longer own - the `fm_autoarm_renewal_in_progress` predicate over live, dead, absent, and malformed markers, and the arm refusing to fork a competing watcher during a live renewal while starting normally once it clears.
