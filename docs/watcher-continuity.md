...
The hook fires on every Stop, and an eligible primary with supervision need admits one home-scoped owner that runs `bin/fm-watch-arm.sh` as a waited child inside the hook-owned process tree.
...
Because Claude Code kills the hook's whole process group at the hook's declared timeout and offers no unbounded value, the owner renews continuity before that bound: it closes its own arm under the pid-matched renewal marker (a `continuity-renewal` lifecycle row, never an anonymous interrupt) and exits 2 with a benign renewal banner, so one bounded turn hands the park to a fresh Stop-owned firing with a fresh timeout budget, while a renewal that leaves another verified live watcher behind closes silently for no turn at all; [`turnend-guard.md`](turnend-guard.md#harness-integrations) owns the bound, margin, and gate details.
...
The watcher's own singleton admission contract - atomic acquisition, the settle re-check that brackets the mid-renewal window, and the statement that a superseded watcher performs no side effect, `watcher_owns_lock` being re-checked at the poll-loop top and again before each commit that follows a multi-second observation span so a superseded watcher stands down and leaves its pending work durable and unmarked for the rightful holder - is stated once in the `bin/fm-watch.sh` header, which is the only place it is spelled out and which owns the list of spans and paths it covers.
...
The same suite covers the atomic-admission ownership gate across the bounded mid-iteration spans - a watcher superseded during the signal grace, one superseded during the triage probe that follows it, one superseded during a pane capture, and one superseded during a stale classify probe, each standing down without enqueuing, absorbing, advancing the seen-marker, or writing the stale suppressor for work they no longer own - plus the `fm_autoarm_renewal_in_progress` predicate over live, dead, absent, and malformed markers, and the arm refusing to fork a competing watcher during a live renewal while starting normally once it clears.
`tests/fm-pending-reply.test.sh` covers that same stand-down contract inside the pending-reply tick alone: a superseded watcher reposts nothing and leaves the record's single recovery budget unspent for the holder.
`tests/fm-supervision-events.test.sh` covers the event-wait splice, including the actionable rc=0 arm that proves the ownership gate is inert with no watcher runtime rather than terminating a sourcing shell, and the negative control that keeps that proof falsifiable: with a superseded owner the same arm stands down at the gate, waking nothing and leaving the backend dedupe marker unwritten for the holder.
