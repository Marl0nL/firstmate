...
Because Claude Code kills the hook's whole process group at the hook's declared timeout and offers no unbounded value, the owner renews continuity before that bound: it closes its own arm under the pid-matched renewal marker (a `continuity-renewal` lifecycle row, never an anonymous interrupt) and exits 2 with a benign renewal banner, so one bounded turn hands the park to a fresh Stop-owned firing with a fresh timeout budget; [`turnend-guard.md`](turnend-guard.md#harness-integrations) owns the bound, margin, and gate details.
