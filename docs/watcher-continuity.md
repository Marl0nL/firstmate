...
The watcher's own singleton admission contract - atomic acquisition, the settle re-check that brackets the mid-renewal window, and the category invariant that no side-effect commit follows a multi-second observation span (a pane capture, a crew-state classify probe, an SSH/backend observe, or the terminal event wait) without an ownership re-check, so a superseded watcher stands down and leaves the pending work durable and unmarked for the rightful holder - is stated once in the `bin/fm-watch.sh` header, which is the only place it is spelled out.
