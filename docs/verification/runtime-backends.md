...
1. Claim-suppression: `bin/fm-spawn.sh` no longer excludes `kind=secondmate`, so a secondmate supervisor's claude launch is also prefixed `HERDR_ENV=0` and its pane is left unclaimed - but only when the secondmate home actually carries an explicit `config/backend=herdr` pin; a home without that pin (a remote mate launched host-locally with the inherit push skipped, or a local home whose pin write failed or was gitignore-guard-skipped) deliberately stays integration-claimed rather than losing its own herdr resolution.
