...
3. First cycle: arm supervision with the `fm_watch_arm_pi` tool.
...
5. Ordinary wake: if the extension says the watcher is already healthy, do not start another cycle.
6. Failure or missing cycle only: if the extension reports a watcher failure, drain queued wakes, inspect the failure text, call `fm_watch_arm_pi`, and restart Pi with both extensions loaded if needed.
