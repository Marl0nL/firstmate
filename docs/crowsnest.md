...
- `state/chat-watch.check-trust` - the trust record binding that shim to its exact bytes. Written and removed with the shim; the watcher refuses an unregistered check instead of running it, so a hand-edited shim must be re-bound with `bin/fm-check-register.sh chat-watch`.
