...
config/public-denylist  optional operator-private strings that must never appear in tracked files; LOCAL, gitignored, not inherited; when present, fm-lint.sh fails on any tracked file containing one (docs/configuration.md "Operator-private denylist")
...
state/               runtime records and signals; gitignored
  ...
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-renewal .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .turnend-claude-blocks .turnend-claude-blocks.lock   Claude Stop auto-arm single-flight, epoch, declared-timeout renewal-handoff, failure-episode, attended-alarm, guard-budget, and budget-lock records; never touch
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The secondmate liveness sweep deterministically accounts for every registered secondmate: it relaunches only from the recovery-grade `dead` or `missing` states, cycles a live claude/herdr mate only when it provably lost its launch flags to a Herdr restore and its home is confirmed idle (`docs/herdr-backend.md`), preserves ambiguous, unreadable, or unreachable remote targets, and reports skipped or failed guarantees as `SECONDMATE_LIVENESS:` lines (`bin/fm-bootstrap.sh`; `bin/fm-backend.sh`'s `fm_backend_agent_state`; `docs/remote-secondmates.md`).
...
Load `secondmate-provisioning` before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a secondmate home, before raising or standing down a wake-resident secondmate, and before editing `data/secondmates.md`.
...
Relay and a wake-resident secondmate may require that same live cycle with no fleet work.
...
- `secondmate-provisioning` - load before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a secondmate home, before raising or standing down a wake-resident secondmate, and before editing `data/secondmates.md`.
