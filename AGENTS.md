...
If the session lock is refused, remain read-only and tell the captain what the digest's banner reports: either another active session is managing the fleet, or this session could not identify its own harness process and no other session holds the lock.
...
When the token-usage monitor is opted in and its advisory guard reports a hold for large low-priority work, keep that dispatch queued as quota-held and re-evaluate when the hot window resets; load `usage-monitor` and never let a quota hold stop an explicit captain dispatch.
...
3. For `check:`, act on the named poll result, including merges, X-mode events, and Crowsnest `chat-mention <id>` wakes.
   A `check: rejected unauthenticated state checks:` wake means those check files were refused without running, so that monitoring is dead until you inspect each file and either re-bind it with `bin/fm-check-register.sh <id>` or remove it; never ignore it to quiet the wake.
...
An unresolved decision that belongs to the captain becomes a durable captain-held item under `decision-hold-lifecycle`, which owns that lifecycle.
...
Without it a call made from inside a project reads that project's absent backlog as empty and writes a stray one into the clone; `bin/` call sites already resolve it through `bin/fm-tasks-axi-lib.sh`.
...
- `usage-monitor` - load before a quota-based hold decision at intake, when producing a token-usage report, on a `usage-quota ...` check wake, or when reading the session-start token-usage line; relevant only when the usage monitor is opted in (`docs/usage-monitor.md`).
